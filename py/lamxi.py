...
for lss in lsd:
    ...
    for m in xrange(nm):
      ast[isn,m]=polyfit(log(fluc[imin:imax,m]),log(perp[imin:imax,m]),1)[0]
    isn=isn+1
avperp=exp(perpav)
avfluc=exp(flucav)
avpar=exp(parav)
for i in xrange(nm):
    alamxi[i]=polyfit(log(avfluc[iminperp:imaxperp,i]),log(avperp[iminperp:imaxperp,i]),1)[0]
    loglog(avfluc[:,i],avperp[:,i],color=rgbl[nm-1-i],linewidth=1.5)
    errfact=2./sqrt(nsn*1.0)
    si[i]=errfact*sqrt(sum((alamxi[i]-ast[:,i])**2)/(1.0*nsn))
#loglog(avpar[iminperp,:],avperp[iminperp,:],'k')
#loglog(avpar[imaxperp,:],avperp[imaxperp,:],'k')

loglog([1e-15*ones(nm),avfluc[15,:],1e15*ones(nm)],avperp[15,:]*(array([1e-15*ones(nm),avfluc[15,:],1e15*ones(nm)])/avfluc[15,:])**alamxi,'k:')
#loglog([1e-15*ones(nm),avpar[15,:],1e15*ones(nm)],avperp[15,:]*(array([1e-15*ones(nm),avpar[15,:],1e15*ones(nm)])/avpar[15,:])**0.5,'k:')

xlabel(r'$\langle (\delta z^+_\perp)^n | \theta_{B_{loc}} = 90\degree,\theta_{\delta z^+} = 0\degree \rangle$',fontsize=30)
...
errorbar(ms,1-alamxi,yerr=si,color='k',linewidth=1.5)
xlabel(r'$n$',fontsize=25)
ylabel(r'$\alpha_n$',fontsize=25)
ylim(0.2,0.5)

savefig('/Users/alfy/3danis_2014/lamxi.eps')
